module Imports
	class OrdinanceAuthorsImporter
		require 'roo'
      ...
      (2..authors_spreadsheet.last_row).each do |i|
        ...
        ActiveRecord::Base.transaction do 
          ...
          if find_member(row).present?
            ordinance.authorships.where(author_id: find_member(row).id).where(author_type: find_member(row).class.name).first_or_create! do |a|
